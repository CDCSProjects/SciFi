...
class Storage{
    ...
    */
    ...
    //adding code for using boost module in pytorch

    int asset_count(){
        meta_store->execQuery("SELECT COUNT(*) FROM filedata");
        std::string count_string = meta_store->getResultAsString();
#ifdef OUTPUTSHELL
        ...
        for(int i = 0; i < count_string.size(); i++) {
            ...
            current += count_string[i];
        }
        int count=std::stoi(current);
        return count;
    }

    PyString getitem(std::string index){
        std::string result = asset_store->getSingle(index);
        PyString pystring = PyString(result);
        return(pystring);
    }

    std::string getIDsByIndex(std::string query){
        meta_store->execQuery(query);
        std::string ids = meta_store->getResultAsString();
        return ids;
    }

    PyString wrapper_get_function(std::string id, int assetToFile = 0, std::string fileextension="", int metaToFile = 0, std::string fileextension_meta="", int dumpmeta=0)
    {
        std::string result;
        result = get(id,assetToFile,fileextension,metaToFile,fileextension_meta);
        PyString pystring = PyString(result);
        return pystring;
    }

    PyString wrapper_get_by_constraint(std::string constraint, int assetToFile = 0, std::string fileextension="", int metaToFile=0, std::string fileextension_meta="")
    {
        std::string result;
        result=get_by_constraint(constraint, assetToFile, fileextension, metaToFile, fileextension_meta);
        PyString pystring=PyString(result);
        return pystring;
    }
