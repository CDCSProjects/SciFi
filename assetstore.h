...
class AssetStore{
    public:
        ...
        virtual std::string getSingle(std::string pdbid) = 0; //This should open the database, get a single value, and close the database again
       ...
       // virtual std::vector<filedata> createportable(std::string directory, int recursive, int pathdepth, int ext, int removeprefixchar=0) = 0;
        ...
        void getSingleToFile(std::string pdbid, std::string fileextension) ; //This should open the database, get a single value, and close the database again
